...
func startServer(t *testing.T) *httptest.Server {
	...
	catalog := persistence.NewCachedProductCatalog(persistence.NewDBProductCatalog(pool))
	if err := catalog.Warm(context.Background()); err != nil {
		t.Fatalf("warm catalog: %v", err)
	}

	router := web.NewRouter(
		web.NewCustomerHandler(application.NewRegisterCustomer(customers, email.NewLogNotifier())),
		web.NewProductHandler(application.NewListProductsByCategory(catalog)),
		pool,
	)
...
func TestCustomerAPI(t *testing.T) {
	...
	t.Run("serves the seeded catalog and sets security headers", func(t *testing.T) {
		...
		if len(products) != 2 {
			t.Fatalf("expected 2 seeded products in c1, got %d", len(products))
		}
	})
}
