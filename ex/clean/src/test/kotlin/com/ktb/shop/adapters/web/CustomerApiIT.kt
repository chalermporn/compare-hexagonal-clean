...
class CustomerApiIT : PostgresSupport() {
    ...
    @Test
    fun `serves the generated OpenAPI spec`() {
        val resp = client.toBlocking()
            .exchange(HttpRequest.GET<Any>("/swagger/clean-shop-api-0.1.0.yml"), String::class.java)
        assertEquals(HttpStatus.OK, resp.status)
        assert(resp.body()!!.contains("Clean Shop API")) { "spec should carry the API title" }
    }
}
